...
const DEFAULTS = {
  ...
  // New Tab
  // On by default — users who never touch the welcome toggle still get the
  // photo new tab. Users who explicitly opt out via the welcome page or
  // Settings have `featureNewtab: false` written to storage, which
  // overrides this default at getConfig() merge time.
  featureNewtab: true,
...
// The scopes used by every feature in the extension. Used both as the
// "recommended" list shown on the welcome / settings scope card and as the
// `permissions` array sent when minting an API key from an existing
// browser session (see `createApiKeyFromSession()` below). Keep this list
// the source of truth — the click-to-copy rows on the welcome and
// settings pages reference the same names.
export const RECOMMENDED_API_KEY_SCOPES = Object.freeze([
  "asset.upload",
  "asset.read",
  "asset.download",
  "asset.update",
...
// Mints a new Immich API key by piggybacking on an existing browser session
// — i.e. a session cookie set when the user signed into Immich (via OAuth,
// SSO, or password) in any normal browser tab. The extension never sees
// the cookie value (it's httpOnly); the browser forwards it automatically
// when we fetch with `credentials: "include"`, and our `<all_urls>`
// host_permission lets that fetch bypass the usual cross-origin block.
//
// Throws a friendly Error with `.status` populated:
//   - status=401 when there's no active session at that host (caller
//     surfaces a "sign in there first" hint with a link)
//   - other statuses for unexpected server responses
//
// On success, returns the API key secret (the long opaque string the user
// would otherwise have copied from Immich → Account Settings → API Keys).
export async function createApiKeyFromSession(serverUrl, {
  scopes = RECOMMENDED_API_KEY_SCOPES,
  name = "Browser Extension (auto-generated)",
} = {}) {
  if (!serverUrl) throw new Error("Server URL required.");
  const cleanUrl = serverUrl.trim().replace(/\/+$/, "").replace(/\/api$/i, "");
  const res = await fetch(`${cleanUrl}/api/api-keys`, {
    method: "POST",
    credentials: "include",
    headers: {
      "Content-Type": "application/json",
  ...
  if (res.status === 401) {
    const e = new Error(`Not signed in at ${new URL(cleanUrl).host}`);
    e.status = 401;
    e.serverUrl = cleanUrl;
    throw e;
  }
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    const e = new Error(`Failed: ${res.status} ${text.slice(0, 120)}`);
    e.status = res.status;
    throw e;
  }
  const data = await res.json();
  const secret = data?.secret;
  if (!secret) throw new Error("Server didn't return a key secret.");
  return { secret, serverUrl: cleanUrl };
}

...
async function fetchWithTimeout(
  ...
  try {
    ...
    if (e?.name === "AbortError") {
      throw new Error(
        `Immich server unreachable (timed out after ${timeoutMs / 1000}s)`,
      );
    }
    ...
    if (
      ...
      /NetworkError|Failed to fetch|net::ERR_/i.test(msg)
    ) {
      ...
      if (isFirefox) {
        ...
        permsHint =
          " On Firefox, also check about:addons → Immich Companion → Permissions → 'Access your data for all websites' is on.";
      }
      throw new Error(
        `Couldn't reach ${host}. Common causes: the server is down, ` +
          ...
          `extension page is loaded over https (mixed-content block).` +
          permsHint,
      );
    }
    throw e;
...
async function request(path, { method = "GET", body, headers = {} } = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Immich ${method} ${path} failed: ${res.status} ${text.slice(0, 200)}`,
    );
  }
...
export async function createAlbum(albumName) {
  return request("/albums", { method: "POST", body: { albumName } });
}
export async function addAssetsToAlbum(albumId, assetIds) {
  return request(`/albums/${albumId}/assets`, {
    method: "PUT",
    body: { ids: assetIds },
  });
}
...
export async function metadataSearch(params, size = 20) {
  return request("/search/metadata", {
    method: "POST",
    body: { ...params, size },
  });
}
...
export async function createShareLink(
  assetIds,
  { expiresAt = null, allowDownload = true, showMetadata = true } = {},
) {
  const body = {
    type: "INDIVIDUAL",

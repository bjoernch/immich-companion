...
async function load() {
  ...
  if ($("newtabRotateSeconds"))
    $("newtabRotateSeconds").value = String(cfg.newtabRotateSeconds || 0);
...
function collect() {
  ...
  for (const k of SELECT_KEYS) {
    if ($(k))
      patch[k] = k === "newtabRotateSeconds" ? Number($(k).value) : $(k).value;
  }
...
$("resetAll").addEventListener("click", async () => {
  const ok = confirm(
    "Wipe every saved setting and reopen the welcome page?\n\n" +
      ...
      "default album, and recent uploads from extension storage on this " +
...
const ISSUE_NEW_BASE =
  "https://github.com/bjoernch/immich-companion/issues/new";
const ISSUE_PICKER_URL =
  "https://github.com/bjoernch/immich-companion/issues/new/choose";
...
function chromiumVersion(uaData, ua, preferred) {
  if (uaData?.brands) {
    ...
    const real = uaData.brands.find(
      (b) => /^\d/.test(b.version) && !/Not.*Brand/i.test(b.brand),
    );
...
async function detectBrowser() {
  ...
  if (
    uaData?.brands?.some((b) => b.brand === "Microsoft Edge") ||
    /Edg\//.test(ua)
  ) {
    return {
      label: "Microsoft Edge",
      version: chromiumVersion(uaData, ua, "Microsoft Edge"),
    };
  }
  if (
    uaData?.brands?.some((b) => b.brand === "Opera") ||
    /OPR\/|Vivaldi/.test(ua)
  ) {
    return {
      label: "Vivaldi / Opera / other Chromium",
      version: chromiumVersion(uaData, ua),
    };
  }
  if (
    ...
    return {
      label: "Google Chrome",
      version: chromiumVersion(uaData, ua, "Google Chrome"),
    };
  }
    ...
    try {
      if (uaData.getHighEntropyValues) {
        ...
        if (hi?.platformVersion)
          str = `${uaData.platform} ${hi.platformVersion}`;
      }
    } catch {}
    return str;
  }
...
function buildBugReportUrl(info) {
  ...
  if (info.extensionVersion)
    params.set("extension-version", info.extensionVersion);

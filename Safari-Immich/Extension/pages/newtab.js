...
function showConnectionErrorOverlay(cfg, error) {
  ...
  let host = "";
  try {
    if (cfg?.serverUrl) host = new URL(cfg.serverUrl).host;
  } catch {}
...
async function showSetupOverlay(reason) {
  ...
  try {
    ...
    diagRow(
      ...
      sKey ? "ok" : "err",
      sKey
        ? `${sKey.slice(0, 4)}…${sKey.slice(-4)} (${sKey.length} chars)`
        : "(empty)",
    );
    diagRow(
      diag,
    ...
    // Also peek at storage.local in case sync is paused/disabled.
    try {
  ...
  if (!isConfigured(cfg)) {
    await showSetupOverlay(
      !cfg.serverUrl && !cfg.apiKey
        ? "Server URL and API key are both empty in extension storage."
        : !cfg.serverUrl
          ? "Server URL is empty."
          : "API key is empty.",
    );
...
$("connErrorSettings")?.addEventListener("click", () =>
  chrome.runtime.openOptionsPage(),
);

...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (chrome.alarms?.create) {
    chrome.alarms.create(ALARM_PING, {
      periodInMinutes: 5,
      when: Date.now() + 1000,
    });
  }
...
chrome.contextMenus?.onClicked?.addListener(async (info, tab) => {
  ...
  try {
    if (
      info.menuItemId === MENU.SAVE_IMAGE ||
      info.menuItemId === MENU.SAVE_VIDEO
    ) {
      await saveUrlToImmich(info.srcUrl, tabId, { share: false });
    } else if (
      info.menuItemId === MENU.SAVE_AND_SHARE_IMAGE ||
      info.menuItemId === MENU.SAVE_AND_SHARE_VIDEO
    ) {
...
async function fetchAssetBytes(url, tabId) {
  ...
  if (
    swErr instanceof TypeError ||
    /NetworkError|Failed to fetch|net::ERR_|CORS/i.test(errMsg)
  ) {
    throw new Error(
...
function fetchInTab(tabId, url) {
  if (!tabId || !chrome.tabs?.sendMessage) return Promise.resolve(null);
...
async function updateBadge() {
  ...
  try {
    ...
    chrome.action.setTitle({
      title: `Immich Companion — connected to ${new URL(cfg.serverUrl).host}`,
    });
  } catch (e) {
    chrome.action.setBadgeText({ text: "!" });
    chrome.action.setBadgeBackgroundColor?.({ color: "#ff6b6b" });
    chrome.action.setTitle({
      title: `Immich Companion — connection failed: ${e.message}`,
    });
  }
}
  ...
  (async () => {
    try {
      if (msg?.type === "thumb") {
        ...
        try {
          res = await fetch(
            thumbnailUrl(cfg.serverUrl, msg.assetId, msg.size || "preview"),
            {
          ...
          if (e?.name === "AbortError") {
            throw new Error(
              "Immich server unreachable (thumbnail fetch timed out after 15s)",
            );
          }
          throw e;
        } finally {
        ...
        if (!chrome.downloads?.download) {
          console.debug(
            TAG,
            "chrome.downloads not available, telling caller to fall back",
          );
          ...
          try {
            ...
            const onChanged = (delta) => {
              ...
              if (finalState === "complete" || finalState === "interrupted") {
                ...
                try {
                  URL.revokeObjectURL(heldBlobUrl);
                } catch {}
                if (finalState === "interrupted") {
                  chrome.downloads.search({ id }, (items) => {
                    ...
                    console.error(
                      TAG,
                      `#${id} interrupted: error=${it?.error} bytesReceived=${it?.bytesReceived} totalBytes=${it?.totalBytes}`,
                    );
                  });
                }
              }
